...
export function ModelSelectDialog({
  ...
  const categorize = (modelList: string[]) => {
    ...
    modelList.forEach(m => {
        ...
        if (!matched) uncategorized.push(m)
    })
  ...
  React.useEffect(() => {
      if (open) {
          if (newModelsData.length > 0) setActiveTab('new')
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col p-0">
                    ...
                    <div className="flex items-center justify-between bg-slate-50 p-2 px-3 rounded-md border sticky top-0 z-10 shadow-sm">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] text-muted-foreground">
                                已选 {cat.models.filter(m => selected.includes(m)).length} / {cat.models.length}
                            </span>
                            <Checkbox 
        ...
        <DialogFooter className="p-6 pt-2 flex items-center justify-between sm:justify-between border-t bg-slate-50">
          <div className="text-sm">
            总计已选择 <span className="font-bold text-blue-600">{selected.length}</span> 个模型
          </div>
