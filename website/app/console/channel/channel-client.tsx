...
export function ChannelClient({ 
  ...
  const handleTagModeChange = (val: boolean) => {
    setTagMode(val)
    const params = new URLSearchParams(searchParams.toString())
    if (val) params.set('tag_mode', 'true')
  ...
  const onBatchDelete = async () => {
    ...
    if (!window.confirm(`确定要删除选中的 ${selectedIds.length} 个渠道吗？`)) return
  ...
  const onBatchTag = async () => {
    if (!selectedIds.length || !newTag) return
  ...
  const onTestAll = async () => {
    // eslint-disable-next-line no-alert
    if (!window.confirm('确定要测试所有通道吗？')) return
    toast.info('正在启动全量测试...')
  ...
  const onUpdateBalances = async () => {
    // eslint-disable-next-line no-alert
    if (!window.confirm('确定要更新所有通道余额吗？')) return
    toast.info('正在更新余额...')
  ...
  const onFixAbilities = async () => {
    // eslint-disable-next-line no-alert
    if (!window.confirm('确定要修复数据库一致性吗？')) return
  ...
  const onDeleteDisabled = async () => {
    // eslint-disable-next-line no-alert
    if (!window.confirm('确定要删除所有已禁用的渠道吗？此操作不可逆！')) return
        ...
        <DialogContent>
          ...
          <div className="py-4 space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">将选中的 {selectedIds.length} 个渠道设置为此标签</p>
            </div>
          </div>
          <DialogFooter>
